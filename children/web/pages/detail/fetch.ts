...
export default async ({ store, router }: Params, ctx?: ISSRNestContext<{
  ...
  const data = __isBrowser__ ? await (await window.fetch(`http://127.0.0.1:3000${window.prefix}/api/detail/${router.params.id}`)).json() : await ctx?.apiDeatilservice?.index(ctx.request.params.id)
