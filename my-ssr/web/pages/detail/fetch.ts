...
export default async ({ store, router }: Params, ctx?: ISSRContext<{
  ...
  const data = __isBrowser__ ? await (await window.fetch(`http://localhost:3000/api/detail/${router.params.id}`)).json() : await ctx?.apiDeatilservice?.index(ctx.request.params.id)
